...
if __name__ == "__main__":
    ...
    if not os.path.exists(NOANOMFILE) and not os.path.exists(ANOMFILE):
        imdf      = utils.getIms(PATH)
        print(imdf.head())
        imdf      = utils.subsetIms(imdf, SUBTYPE)
        reference = utils.buildReference(PATH, imdf)
        refim     = utils.getRefIm(imdf)
        splices   = utils.createSplices(PATH, refim, mode='feature', dim=64, k=4)
        partimgs  = utils.imPartition(PATH, imdf, reference, splices)
        ...
        utils.saveParts(parts, ANOMPATH, NOANOMPATH)
    ...
    part0, part1 = utils.underSamp(part0, part1)                                    # 80:20 distribution, by default
